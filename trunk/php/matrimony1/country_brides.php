...
$sql = "SELECT * FROM users, user_profile, countries, religion WHERE users.UserID=user_profile.UserID and users.CountryID=countries.CountryID and users.ReligionID=religion.ReligionID and users.Status=1 and users.ApprovalStatus=1 and users.Gender='Female'  and ".$religion." order by users.UserID desc limit $eu, $limit";

$sqltot = "SELECT * FROM users, user_profile, countries, religion WHERE users.UserID=user_profile.UserID and users.CountryID=countries.CountryID and users.ReligionID=religion.ReligionID and users.Status=1 and users.ApprovalStatus=1 and users.Gender='Female'  and ".$religion." order by users.UserID desc";
...
<html>
<head>
<title>Marry Banjara - Search Brides by Country</title>
			...
			<div style="border-top: 1px solid rgb(143, 167, 191); border-left: 1px solid rgb(143, 167, 191); border-right: 1px solid rgb(143, 167, 191);">
				...
				<div style="margin: 0px; width: 100%;">
					<div style="width: 180px; background-color: rgb(255, 255, 255); float: left;">
						...
						<div style="padding: 6px 0pt 0pt 0px; width: 170px; background-color: rgb(238, 238, 238);" class="smallblack"><div>
						<?PHP
						if($_SESSION['UserID']!="")
						{
						?>
...
if(@mysql_num_rows($resultstate) != 0)
{
 echo $rowstate['State'].", ";
	...
	?> <?PHP echo stripslashes($row['Country'])?></td>
		<td class="mediumblack" style="border-bottom: 1px solid rgb(216, 241, 186); padding-left: 15px; padding-right: 10px;" bgcolor="#FFE8E8" width="321" align="left"><?PHP
