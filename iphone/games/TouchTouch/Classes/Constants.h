//
//  Constants.h
//  TouchTouch
//
//  Created by sandeep m on 23/10/2010.
//  Copyright 2010 bri. All rights reserved.
//

#import <Foundation/Foundation.h>
#define DEFAULT_BACKGROUND @"wood2.jpg"
#define DEFAULT_BUTTON_IMAGE @"button.png"
#define DEFAULT_FONTNAME @"AmericanTypewriter"
#define DEFAULT_FONTNAME_BOLD @"AmericanTypewriter-Bold"
#define COLOR_DARKBROWN 0x3B1701

#define BACKGROUND_BUTTERFLY @"background-nature.jpg"
#define BACKGROUND_BALLOON @"background-sky.png"
#define BACKGROUND_UFO @"background-mars.jpg"

@interface Constants : NSObject {

}

@end
