...
class insert_image extends matrimony
{
	function insert(){
		...
		$argVal="'".$_SESSION['ProfileId']."','$this->Image_Id','$this->imageName','I',now(),now(),'$Thumb_image','$Enl_image','new'";
		...
		$thumb = new thumbnail($directory,100,100);
		$this->enlarge();
	}
	function update(){
		$value=explode(".",$this->imageName);
	...
	function cropupdate(){
		...
		$thumb = new thumbnail($filename,100,100);
		$thumbimg = explode("/",$thumb);
		$fields=" image_thumb='$thumbimg[1]',Image_Content='update',Image_DOM=now() where Image_ID='".$_POST['hdimgid']."'";
		$this->switchqry('tm_image','UPDATE',$fields,'');
	}
	function deleteImage(){
		...
		unlink($file."/".$Image_Name);
		unlink($file."/".$image_thumb);
		unlink($file."/".$image_small);
		$whrarg=" where Image_ID = '".$_POST['img_id']."'";
		$this->switchqry("tm_image",'DELETE',$whrarg,'');
	}
	function select(){
		$whrarg = " WHERE Profile_ID = '".$_SESSION['ProfileId']."'";
		$this->switchqry('tm_image','SELECT',$whrarg,'*');
		$i = 0;
	...
	function insertpassword(){
		$pass=$_POST['txtPwd'];
		$fields="Image_Password = '$pass'  where Profile_ID = '".$_SESSION['ProfileId']."'";
		$this->switchqry('tm_image','UPDATE',$fields,'');
	}
	function getCount(){
	...
	function enlarge(){
		...
		header("location:managephoto.php");
	}
}
